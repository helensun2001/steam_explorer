...
from src.visualization import plot_developer_barchart
...
game_tags = Read_csv_to_list("data/tags_for_exploration.csv")
data = pd.read_csv("data/games_to_recommend.csv")
...
# 获取所有发行商
developer = ['All'] + data['developer'].unique().tolist()
# 发行商选择框
selected_developer = st.selectbox("Select a developer:", developer)
...
if st.button("EXLPORE"):
    ...
    if selected_developer == 'All':
        ...
        filtered_data = data[data['developer'] == selected_developer]
        if 'All' in selected_games:
    ...
    if tag_slice.shape[0] == 0:
        ...
        with col3:
            ...
            try:
                ...
                display_metric('Average positive ratings','Data Missing')
        with col4:
            average_price_selected = round(np.mean(tag_slice[tag_slice['price']>0]['price']),1)
            display_metric('Average price',f'{average_price_selected} $')

        if 'All' in selected_games: #没有选标签
        ...
        # plot distribution of developers
            ...
            st.write('\n\n\n')
            plot_mean_playtime_by_owners(tag_slice)
            plot_developer_barchart(tag_slice)
